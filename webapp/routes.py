...
def worklist():
    ...
    test_reason_opts      = ["Pre-Employment","Random","Post Accident","Reasonable Suspicion",
                             "Return To Duty","Follow-Up","Pre-Assignment","Job Requirement",
                             "CDL Recertification","Recertification","Other"]
    test_type_opts        = ["Lab Based Urine Test","Lab Based Hair Test",
                             "Alcohol Breath Test","POCT Urine Test","Physical"]
    test_result_opts      = ["Negative","Negative-Dilute","Non-Negative",
                             "Positive","Cancelled","Lab Reject","Refusal","Other"]
    regulation_opts       = ["DOT","Non-DOT"]
    regulation_body_opts  = ["FMCSA","PHMSA","FTA"]
    positive_for_opts     = ["Unable To Contact","Marijuana","Cocaine","Phencyledine",
                             "Heroin","Amphetamines","PCP"]
    laboratory_opts       = ["Clinical Reference Laboratory","Abbott Toxicology",
...
def worklist_bulk_update():
    ...
    with SessionLocal() as db:
        for item in db.query(WorklistStaging).filter(WorklistStaging.ccfid.in_(ccfids)):
...
def worklist_bulk_send():
    ...
    records = []
    with SessionLocal() as db:
        for item in db.query(WorklistStaging).filter(WorklistStaging.ccfid.in_(ccfids)).all():
            records.append({
                "CCFID":           item.ccfid,
    ...
    if failed:
        flash(f"{failed} records failed to send.", "error")
...
def worklist_detail(ccfid):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if item.collection_site:
                ...
                logger.info("Created %d new collection sites", len(set(full_map) - existing))

            # 3) build lookup maps
            company_map = { c.account_code: c.account_id.replace("zcrm_", "")
                            for c in db.query(Company).all() }
            site_map    = { s.Collection_Site_ID: s.Record_id.replace("zcrm_", "")
                            for s in db.query(CollectionSite).all() }
            lab_map     = { l.Laboratory: l.Record_id.replace("zcrm_", "")
    ...
    with SessionLocal() as db:
        ...
        rows = ( db.query(CollectionSite.Collection_Site,
                          CollectionSite.Collection_Site_ID)
                  .filter(CollectionSite.Collection_Site.isnot(None))
                  .filter(CollectionSite.Collection_Site != "")
                  .distinct()
    ...
    # same picklists as worklist view
    test_reason_opts      = ["Pre-Employment","Random","Post Accident","Reasonable Suspicion",
                             "Return To Duty","Follow-Up","Pre-Assignment","Job Requirement",
                             "CDL Recertification","Recertification","Other"]
    test_type_opts        = ["Lab Based Urine Test","Lab Based Hair Test",
